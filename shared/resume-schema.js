(function () {
  ...
  const SECTION_DEFINITIONS = [
    {
      ...
      fields: [
        ...
        { key: "height", label: "身高（cm）", input: "text", placeholder: "175" },
        { key: "weight", label: "体重（kg）", input: "text", placeholder: "65" },
    ...
    },
    {
      ...
      fields: [
        ...
        { key: "recruitmentSource", label: "招聘信息来源", input: "text", placeholder: "校园招聘网站" },
    ...
    },
    {
      ...
      slots: 16,
      itemLabel: "竞赛获奖",
      note: "与校园经历独立；每个竞赛奖项单独填写一条，网页重复卡片会按顺序对应。",
      fields: [
        { key: "awardDate", label: "获奖时间", input: "date" },
        { key: "name", label: "奖项名称", input: "text", placeholder: "互联网+大学生创新创业大赛" },
        { key: "level", label: "奖项等级", input: "text", placeholder: "国家级 / 省级 / 校级" },
        { key: "rank", label: "奖项名次", input: "text", placeholder: "一等奖 / 二等奖 / 金奖" },
        { key: "issuer", label: "颁发机构", input: "text", placeholder: "教育部 / 学校 / 赛事组委会" },
        { key: "description", label: "获奖描述", input: "textarea", placeholder: "项目、个人贡献或获奖背景" },
      ],
    },
    {
      key: "certificates",
  ...
  const FIELD_VALUE_ALIASES = {
    personal: {
      ...
      ethnicity: ["ethnicGroup", "ethnic", "民族"],
      height: ["heightCm", "bodyHeight", "身高"],
      weight: ["weightKg", "bodyWeight", "体重"],
    },
    contactAndLocation: {
    ...
    jobPreferences: {
      ...
      recruitmentSource: ["recruitmentChannel", "jobSource", "sourceChannel", "招聘来源", "信息来源"],
    },
    educations: {
    ...
    awards: {
      awardDate: ["date", "time", "awardTime", "issueDate", "获奖时间"],
      name: ["awardName", "title", "prizeName", "奖项", "奖项名称"],
      level: ["awardLevel", "grade", "scope", "等级", "奖项等级"],
      rank: ["awardRank", "prize", "placement", "名次", "奖项名次"],
      issuer: ["organization", "awardingBody", "颁发机构"],
      description: ["details", "summary", "获奖描述"],
    },
  };
  ...
  function appendEmptyListItem(profile, sectionKey) {
    const section = getSectionDefinition(sectionKey);
    const nextProfile = clone(profile && typeof profile === "object" ? profile : {});
    if (!section || section.type !== "list") return nextProfile;

    const items = Array.isArray(nextProfile[sectionKey])
      ? nextProfile[sectionKey]
      : [];
    if (items.length >= getListSectionMaxItems(section)) return nextProfile;

    items.push(createEmptyListItem(sectionKey));
    nextProfile[sectionKey] = items;
    return nextProfile;
  }
  ...
  function splitLegacyAwardText(value, maxItems) {
  ...
  function isLegacyCampusCompetition(value) {
    const category = normalizeForMatch(value?.category || value?.type || "");
    return ["竞赛", "competition", "contest"].includes(category);
  }

  function createAwardFromCampusCompetition(item) {
    return {
      ...createEmptyListItem("awards"),
      awardDate: item?.endDate || item?.startDate || "",
      name: item?.organization || "",
      rank: item?.achievements || "",
      description: item?.description || "",
    };
  }

  function createAwardIdentity(item) {
    return [item?.name, item?.awardDate, item?.rank]
      .map((value) => normalizeForMatch(value))
      .join("|");
  }

  function mergeAwardRecords(records, maxItems) {
    const merged = [];
    const indexByIdentity = new Map();

    for (const record of records) {
      if (!isMeaningfulValue(record)) continue;

      const identity = createAwardIdentity(record);
      const existingIndex = identity && indexByIdentity.get(identity);
  ...
  function migrateLegacyAwards(source, profile) {
    const awardSection = getSectionDefinition("awards");
    const maxItems = getListSectionMaxItems(awardSection);
    const sourceCampus = Array.isArray(source?.campusExperiences)
      ? source.campusExperiences
      : [];
    const campusAwards = [];
    const remainingCampus = [];

    for (let index = 0; index < profile.campusExperiences.length; index += 1) {
      const item = profile.campusExperiences[index];
      if (isLegacyCampusCompetition(sourceCampus[index])) {
        campusAwards.push(createAwardFromCampusCompetition(item));
      } else {
        remainingCampus.push(item);
      }
    }

    profile.campusExperiences = remainingCampus.length
      ? remainingCampus
      : [createEmptyListItem("campusExperiences")];

    const legacyNames = splitLegacyAwardText(
      [source?.additional?.awards, source?.additional?.competitions]
        .filter(Boolean)
        .join("\n"),
      maxItems
    );
    const legacyAwards = legacyNames.map((name) => ({
      ...createEmptyListItem("awards"),
      name,
    }));

    const mergedAwards = mergeAwardRecords(
      [...profile.awards, ...campusAwards, ...legacyAwards],
      maxItems
    );
    profile.awards = mergedAwards.length
      ? mergedAwards
      : [createEmptyListItem("awards")];
  }
  ...
  function normalizeResumeProfile(input) {
    ...
    migrateLegacyAwards(source, profile);
