...
const resumeBackupFileEl = document.getElementById("resumeBackupFile");
...
const resumeTransfer = window.ResumeTransfer;
if (!resumeTransfer) {
  throw new Error("Resume transfer is not available");
}

const localAwardImport = window.LocalAwardImport;
if (!localAwardImport) {
  throw new Error("Local award import is not available");
}
...
function addResumeListItem(sectionKey) {
  ...
  const previousCount = Array.isArray(nextProfile[sectionKey])
    ? nextProfile[sectionKey].length
    : 0;
  resumeProfile = schema.appendEmptyListItem(nextProfile, sectionKey);
  const items = Array.isArray(resumeProfile[sectionKey])
    ? resumeProfile[sectionKey]
    : [];
...
exportResumeBtn.addEventListener("click", async () => {
  try {
    const stored = await resumeStorage.loadResumeData();
    resumeTransfer.downloadTransferPayload(
      resumeTransfer.createTransferPayload(stored),
      "resume-backup"
    );
    updatePageStatus("success", "资料已导出为本地 JSON 备份文件。");
  } catch (error) {
...
importBackupBtn.addEventListener("click", () => {
  resumeBackupFileEl.value = "";
  resumeBackupFileEl.click();
});

resumeBackupFileEl.addEventListener("change", async () => {
  const file = resumeBackupFileEl.files?.[0];
  if (!file) return;

  exportResumeBtn.disabled = true;
  importBackupBtn.disabled = true;
  updatePageStatus("info", `正在导入旧版资料：${file.name}`);

  try {
    const imported = resumeTransfer.parseTransferPayload(await file.text());
    const current = await resumeStorage.loadResumeData();
    resumeTransfer.downloadTransferPayload(
      resumeTransfer.createTransferPayload(current),
      "resume-backup-before-import"
    );

    const normalized = schema.normalizeResumeProfile(imported.profile);
    await resumeStorage.saveResumeData({
      profile: normalized,
      schemaVersion: schema.version,
    ...
    resumeImportTextEl.value = imported.rawText;
    resetCollapsedResumeSections();
    ...
    updatePageStatus(
      "success",
      `旧版资料导入完成，共恢复 ${countFilledSummaryItems(normalized)} 个有效字段；导入前资料也已自动备份。`
    );
  } catch (error) {
